...
firebase.database().ref(`TradeNow/Laptop`).on('child_added',function(data){
...
Description = data.child("Description").val().replace(/<br\s*\/?>/gi,'&nbsp;').replace(/(\r\n\t|\n|\r\t)/gm,"");;
...
if(data.child("Description").val().length >= 65){
   ...
   Description =  data.child("Description").val().replace(/<br\s*\/?>/gi,'&nbsp;').replace(/(\r\n\t|\n|\r\t)/gm,"").replace(/[^\w\s]/gi, '').slice(0,60)+'....';

   
}else if(data.child("Description").val().replace(/<br\s*\/?>/gi,'&nbsp;').length <= 65){

    Description = data.child("Description").val().replace(/[^\w\s]/gi, '').replace(/<br\s*\/?>/gi,'&nbsp;').replace(/(\r\n\t|\n|\r\t)/gm,"");
};
