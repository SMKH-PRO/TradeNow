...
firebase.database().ref(`TradeNow/Car`).on('child_added',function(data){
...
if(data.child("Description").val().length >= 65){
   ...
   Description =  data.child("Description").val().replace(/<br\s*\/?>/gi,'&nbsp;').slice(0,60)+'....';
    ...
    Description = data.child("Description").val().replace(/<br\s*\/?>/gi,'&nbsp;'); //Break lines are not needed in card view
};
